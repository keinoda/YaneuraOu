...
- 状態: ユーザー判断で2,040局時点に打ち切り。SPRT境界未到達で、再開しない
...
- workload size: 32
- 局数: 2,040
- W/L/D: `767 / 872 / 401`（dev視点）
- pentanomial: `179 / 156 / 409 / 143 / 133`
- 最終LLR: `-1.5237`（境界`[-2.2513, 2.8904]`には未到達）
- Elo: `-17.90 ± 13.17` (95%)

base/devとも`nmpMinPly`を探索開始時に初期化するため、固定値SPRTの判断には
#72だけを用いる。ただし、#72はSPSA前の固定値を差し込んだ途中結果であり、
この値だけで採否を決めない。変更後のSPSAへ進むため、ユーザー判断で打ち切っており、
再開しない。
...
使用せず、削除済みテストを復元しない。修正版同士の#72を固定値試験の正本とするが、
#72も境界未到達で打ち切っているため、採否の根拠にはしない。

## 再SPSA後の最終評価

未実施。ShogiBenchのSPSA経路試験#75が完了した後、本ブランチに対して同じ
148項目の軽量SPSAを行う。軽量SPSAは経路と傾向の確認であり、結果をmasterへ
反映しない。有望な場合だけ本調整規模のSPSAへ進み、調整に使用していない対局で
現行masterと比較する。masterへの反映は、結果提示後にユーザーから明示的な許可を
得た場合に限る。それまではブランチを保持し、総合判断で不採用となった場合は
ユーザー方針に従いブランチを削除する。
