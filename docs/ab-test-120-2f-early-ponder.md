...
### 追加対応: 現在手のincrement先取りを廃止

このブランチでは、incrementは着手完了後に加算される時間として扱い、現在手の
消費可能上限を次の式へ変更した。

```
remain_time = time + byoyomi - NetworkDelay2
```

これにより、`btime/wtime`へincrementが反映済みかどうかにかかわらず、現在手で
`inc`を追加消費しない。incrementを差し引いた時計を送るクライアントでは現在手の
思考時間が保守的になるが、使わなかった時間は次手以降へ残る。将来手の予算を
見積もる`inc * MTG`は、着手完了後に得られるincrementなので維持する。
