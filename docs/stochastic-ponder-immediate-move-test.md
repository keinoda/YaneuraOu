...
Stochastic Ponderは、旧探索のbestmoveを抑止して停止し、
実局面から通常探索を再開する。再探索の `startTime` は
ponderhit受信時に固定し、停止・局面復元の時間も対局時計に課金する。

通常Ponderは2026-07-13にV8.30基準へ戻した。引数なしだけでなく、
時刻付き `ponderhit` でも探索を停止・再起動せず、同じroot、
反復深化、PV、nodes、root統計を維持する。時計付きhitでは時間制御だけを
探索メインスレッド上で再初期化し、hit受信時刻から残り予算を計る。
...
ponder missをPTYで検証し、不適合時は非0で終了する。通常Ponderでは
hit前後の `depth` と `nodes` も比較し、内部再探索への退行を検出する。
`Stochastic_Ponder` は `getoption` で設定値を読み戻すだけでなく、Ponder中の
PV先頭手を検査する。ONでは最後の1手を戻した局面、OFFでは実局面の合法手集合に
属することを確認するため、Stochasticの実局面再探索が
通常Ponderの継続経路に混ざっても合格しない。
また各応答後0.5秒と次ケース開始前の出力を検査し、停止した旧探索から遅れて出る
二重 `bestmove` も失敗として扱う。通常の0.3秒Ponderと待機なしhitで全件合格した。

時刻更新はmutex保護のpending eventでUSIスレッドから渡し、
探索メインスレッドが `TimeManagement` の再初期化と `ponder=false` を
この順序で行う。V8.30の同一探索継続を保ちつつ、非atomicデータの競合を避ける。
