...
2. **full image**(private image、タグ名は省略): iShogi リポジトリの
   ...
   (§2 の必須パッチと同内容)。検証は LS_PROGRESS_COEFF / FV_SCALE 28 で readyok 確認。
...
リポジトリの `source/progress.bin` は、1024×16×64 の progress / progress8ek
評価関数で使う係数で、SHA256 は
`e7ed0eef88868335f9a46c58a121dccb5ad82a5eb1c8ee12de90365ab351e37d`。
progress8ek の相入玉判定はこの係数を参照せず9番目のLayerStackを選び、それ以外の
局面だけが同じ係数で0..7へ分岐する。
...
- **バケット選択は玉位置(kingrank9)ではなく進行度**。本リポジトリでは
  `source/tanuki_progress.cpp` の `Tanuki::Progress::LayerStackIndex()` が
  9分岐(kingrank9)を置換する実装になっている(外部の系統では同じ分割を
  "progress8kpabs" と呼ぶ)。選択方式は実行時オプション `LS_BUCKET_MODE` で
  決まり、edition 名はレイアウト(バケット数)しか表さない。このため
  edition 名の末尾トークンはバケット数のみを表す **`_ls9` を推奨**する
  (`_k3k3` / `_king3_by_king3` は同一レイアウトのalias(旧称)で、生成される
  ヘッダ内容・バイナリとも `_ls9` と同一)。
...
- ビルドコマンド(Linux x86_64 / AVX512VNNI):
  ...
  make -j"$(nproc)" tournament \
    YANEURAOU_EDITION=YANEURAOU_ENGINE_SFNN_halfkahm2_1536_15_32_ls9 \
  ...
  - edition 名を Makefile が Python ジェネレータに渡し、
    `architectures/SFNN_halfkahm2_1536_15_32_ls9.h` を動的生成する。
    旧名 `..._k3k3` を指定した場合もヘッダ内容・バイナリは同一
    (2026-07-04 に生成ヘッダのdiffなし・`usi` 出力一致を確認済み)。
...
エンジンは `isready` の際に `EvalDir/eval_options.txt` を読み込む。
`LS_PROGRESS_COEFF` の既定値は `progress.bin` であり、ディレクトリを含まない
ファイル名は `EvalDir` を基準に解決される。このため、`eval/progress.bin` を置く
通常構成では係数パスの指定は不要で、内容は次の1行でよい:

```
FV_SCALE 28
```

別の係数を使う場合は `LS_PROGRESS_COEFF` にディレクトリ付きパスまたは絶対パスを
指定する。進行度係数は常に外部ファイルから読み込み、`<internal>` は使用できない。
...
- `SH11235/YaneuraOu` の `feat/accumulator-caches-progress8kpabs` ブランチは
  ...
  `LS_BUCKET_MODE progress8kpabs` + `LS_PROGRESS_COEFF` という旧原典。
  現行の本構成バイナリも進行度係数パスは `LS_PROGRESS_COEFF` なので、
  名前だけを根拠に別系統と判定しないこと。
