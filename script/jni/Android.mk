...
# example: SFNN without PSQT 1536
# $ ndk-build YANEURAOU_EDITION=YANEURAOU_ENGINE_SFNN1536

# example: EVAL_NNUE_HALFKP_512X2_8_64 (unknown architecture)
# $ ndk-build make_nnue_header YANEURAOU_EDITION=YANEURAOU_ENGINE_NNUE_HALFKP_512X2_16_32
# $ ndk-build YANEURAOU_EDITION=YANEURAOU_ENGINE_NNUE_HALFKP_512X2_16_32
...
# YANEURAOU_ENGINE_ 以降を architecture 名として扱う。
ENGINE_ARCH       := $(patsubst YANEURAOU_ENGINE_%,%,$(YANEURAOU_EDITION))
ENGINE_ARCH_UPPER := $(shell printf '%s' '$(ENGINE_ARCH)' | tr '[:lower:]' '[:upper:]')
...
ifneq (,$(IS_NNUE_EDITION))
  ...
  ifeq ($(ENGINE_ARCH_UPPER),NNUE)
    ...
    ifeq ($(ENGINE_ARCH_UPPER),NNUE_KP256)
      ...
      ARCH_GEN_SCRIPT = $(PYTHON) source/eval/nnue/architectures/nnue_arch_gen.py $(NNUE_ARCH) source/eval/nnue/architectures

      # SFNN_* はPSQTなしSFNNなので、シンボル SFNNwoPSQT を定義する。
      ifneq (,$(filter SFNN%,$(NNUE_ARCH_UPPER)))
