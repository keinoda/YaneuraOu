...
return 8143 * pcv + 8088 * micv + 8546 * (wnpcv + bnpcv) + 11377 * cntcv;
...
shared.pawn_correction_entry(pos).at(us).pawn << bonus;
    shared.minor_piece_correction_entry(pos).at(us).minor << bonus * 135 / 128;
    ...
    if (m.is_ok())
    {
        ...
        (*(ss - 2)->continuationCorrectionHistory)[pc][to] << bonus * 141 / 128;
        (*(ss - 4)->continuationCorrectionHistory)[pc][to] << bonus * 53 / 128;
    }
		...
		*/
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
			...
			*/
            ...
            delta     = 18 + threadIdx % 8 + std::abs(rootMoves[pvIdx].meanSquaredScore) / 7883;
...
// reductions tableの初期化(これはWorkerごとが持つように変更された)
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int(2855 / 128.0 * std::log(i));
        ...
        if (ttData.move && ttData.value >= beta)
        {
			...
			*/
			...
			if (prevSq != SQ_NONE && (ss - 1)->moveCount <= 4 && !priorCapture)
                update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq, -1255);
...
int evalDiff = std::clamp(-int((ss - 1)->staticEval + ss->staticEval), -271, 163) + 44;
        mainHistory[~us][((ss - 1)->currentMove).raw()] << evalDiff * 8;
...
if (!ttHit && type_of(pos.piece_on(prevSq)) != PAWN
	...
	*/
    ...
    if (priorReduction >= 2 && depth >= 2 && ss->staticEval + (ss - 1)->staticEval > 179)
        depth--;
...
if (!PvNode && eval < alpha - 568 - 216 * depth * depth)
	...
	*/

    {
		...
		auto futility_margin = [&](Depth d) {
            Value futilityMult = 55 - 23 * !ss->ttHit;

            return futilityMult * d                                //
                 - (2686 * improving + 362 * opponentWorsening) * futilityMult / 1024
                 + std::abs(correctionValue) / 146536;
        };
    ...
    if (cutNode && ss->staticEval >= beta - 22 * depth - 53 * improving + 290 && !excludedMove
...
probCutBeta = beta + 111 - 77 * improving;
...
Value futilityValue = ss->staticEval + 421 + 436 * lmrDepth
                                        + PieceValue[capturedPiece] + 356 * captHist / 1024;
...
if (history < -3095 * depth)
                    continue;

                history += 47 * mainHistory[us][move.raw()] / 32;

				// (*Scaler): Generally, lower divisors scales well
				// 一般に、割る数（divisor）が小さいほどスケールしやすい。

                lmrDepth += history / 3191;



                
Value futilityValue = ss->staticEval + 42 + 151 * !bestMove + 85 * lmrDepth
                                    + 78 * (ss->staticEval > alpha);
...
Value singularBeta  = ttData.value - (64 + 95 * (ss->ttPv && !PvNode)) * depth / 41;
...
int corrValAdj   = std::abs(correctionValue) / 268417;


                
int doubleMargin = 0 + 187 * PvNode - 225 * !ttCapture - corrValAdj
                                 - 757 * ttMoveHistory / 116517 - (ss->ply > rootDepth) * 26;


                
                int tripleMargin = 113 + 393 * PvNode - 75 * !ttCapture + 127 * ss->ttPv - corrValAdj
                                 - (ss->ply > rootDepth) * 42;
...
if (ss->ttPv)
            r -= 2308 + PvNode * 1230 + (ttData.value > alpha) * 781
               + (ttData.depth >= depth) * (843 + cutNode * 1113);
...
r += 977;  // Base reduction offset to compensate for other tweaks
...
r -= moveCount * 27;
        r -= std::abs(correctionValue) / 28654;
...
if (ttCapture)
        ...
        if ((ss + 1)->cutoffCnt > 1)
            r += 251 + 1014 * ((ss + 1)->cutoffCnt > 2) + 562 * allNode;
...
if (capture)
            ss->statScore = 789 * int(PieceValue[pos.captured_piece()]) / 128
...
update_continuation_histories(ss, movedPiece, move.to_sq(), 644);
...
// Note that if expected reduction is high, we reduce search depth here
			...
			value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                                   newDepth - (r > 2508) - (r > 4214 && newDepth > 2), !cutNode);
...
if (!PvNode)
            ttMoveHistory << (bestMove == ttData.move ? 821 : -1008);
...
futilityBase = ss->staticEval + 123;
...
int Search::YaneuraOuWorker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return reductionScale - delta * 801 / rootDelta + !i * reductionScale * 344 / 512 + 1017;
}
...
void update_all_stats(const Position&          pos,
    ...
    int bonus =
      std::min(155 * depth - 54, 1529) + 439 * (bestMove == ttMove) + (ss - 1)->statScore / 32;
    int malus = std::min(560 * depth - 207, 2122);
...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
    ...
    static constexpr std::array<ConthistBonus, 6> conthist_bonuses = {
      {{1, 1022}, {2, 674}, {3, 277}, {4, 679}, {5, 93}, {6, 876}}};
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::YaneuraOuWorker& workerThread, Move move, int bonus) {
    ...
    if (ss->ply < LOW_PLY_HISTORY_SIZE)
        workerThread.lowPlyHistory[ss->ply][move.raw()] << bonus * 742 / 1024;

    update_continuation_histories(ss, pos.moved_piece(move), move.to_sq(), bonus * 833 / 1024);



	
	workerThread.sharedHistory.pawn_entry(pos)[pos.moved_piece(move)][move.to_sq()]
      << bonus * (bonus > 0 ? 806 : 519) / 1024;
